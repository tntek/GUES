...
#             feats = torch.nn.functional.normalize(feats, dim=1, p=2)  # along channel
        ...
        # all_output = F.softmax(output_prob, dim=-1).cpu().numpy()
        ...
        if self.dataset == 'VisDA':
            ...
            y_predict = torch.squeeze(predict).float().cpu().numpy()
            # recall = recall_score(y_test, y_predict,average='macro')
            f1 = f1_score(y_test, y_predict, average='macro')
            auc = roc_auc_score(y_test, output_prob.cpu().numpy(), average='macro', multi_class='ovo')
            q_kappa = cohen_kappa_score(y_test, y_predict, weights='quadratic') 

            self.logger.info('f1: {:.2%} auc: {:.2%} q_kappa: {:.2%}'.format(f1, auc, q_kappa))
            # print(f1)
            # print(auc)
