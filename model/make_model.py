...
class build_transformer(nn.Module):
    ...
    def forward(self, x, cam_label= None, view_label=None, return_logits=False):  # label is unused if self.cos_layer == 'no'
...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(768 * 1, 2048),
            nn.ReLU(),
        )
        self.mean = nn.Linear(2048, 768)
        self.log_var = nn.Linear(2048, 768)
        
        self.apply(weights_init)
        
    def forward(self, x, a):
#         x = torch.cat([x, a], dim=1)
        x = self.net(x)#[256,2048]
        mean = self.mean(x)#[256,512]
        log_var = self.log_var(x)#[256,512]
        return mean, log_var
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.Linear(768 * 1, 4096),
            nn.ReLU(),
            nn.Linear(4096, 768 * 1),
        )
        self.apply(weights_init)
    
    def forward(self, x):
        out = self.net(x)#[256,512]
        return out
...
def vae_net(netE,netG,feat,feat2):
    mean, log_var = netE(feat,feat2)
    std = torch.exp(0.5 * log_var)
    z = torch.randn(mean.shape).cuda()
    z = std * z + mean
    bias = netG(z)
    loss_sou2tar = vae_loss(bias, feat2, mean, log_var)
    return bias,loss_sou2tar
...
class build_uda_transformer(nn.Module):
    ...
    def forward(self, x, x2, label=None, cam_label= None, view_label=None, domain_norm=False, return_logits=False, return_feat_prob=False, cls_embed_specific=False,vae_re=False):  # label is unused if self.cos_layer == 'no'
        ...
        # if vae_re:
        #     bias_1,loss_sou2tar = vae_net(self.netE,self.netG,feat,feat2)
        #     # bias_1,loss_sou2tar = vae_net(self.netE,self.netG,feat,feat)
        #     feat = 0.8*feat + 0.2*bias_1
        #     bias_2,loss_tar2sou = vae_net(self.netE,self.netG,feat2,feat)
        #     # bias_2,loss_tar2sou = vae_net(self.netE,self.netG,feat2,feat2)
        #     feat2 = 0.8*feat2 +0.2*bias_2
        ...
        if self.training or return_feat_prob:
            ...
            # if vae_re:
            #     return (cls_score, global_feat, feat), (cls_score2, global_feat2, feat2), (cls_score3, global_feat3, feat3), cross_attn,loss_sou2tar,loss_tar2sou   # source , target , source_target_fusion
...
def make_model(cfg, num_class):
    if cfg.MODEL.NAME == 'transformer':
        if cfg.MODEL.BLOCK_PATTERN == '3_branches':
            model = build_uda_transformer(num_class, cfg, __factory_hh)
